...
- Desventajas
  - Es necesario parar Oracle para hacer la copia y la restauración

*** Lista de ficheros a copiar

#+BEGIN_SRC sql
select datafile from v$datafile
union
select datafile from v$tempfile
union
select datafile from v$logfilexs
union
select datafile from v$controlfile
#+END_SRC
