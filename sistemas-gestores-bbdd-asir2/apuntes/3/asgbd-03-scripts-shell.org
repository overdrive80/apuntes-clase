...
| Si el servicio ~A~ ... | ~A~ intenta arrancar ~B~ | Si se apaga ~B~, ~A~ también | ~A~ arranca si ~B~ está inicialmente apagado | Si falla arranque de ~S~, arranca ~A~ |
|------------------------+--------------------------+------------------------------+----------------------------------------------+---------------------------------------|
...
| Si el servicio ~A~ |                                            |
|--------------------+--------------------------------------------|
| ~WantedBy=S~       | Equivalente a que ~S~ indique ~Wants=A~    |
| ~RequiredBy=S~     | Equivalente a que ~S~ indique ~Required=A~ |
...
muestra_dialogo(){
  ...
  numerolineas=$(lista_de_opciones | wc -l)


  whiptail --title "Ejemplo whiptail" --menu "Elige un fichero para mostrar" $numerolineas 40 $numerolineas $(lista_de_opciones)
}

elige_fichero(){
  opcion=$(muestra_dialogo 3>&2 2>&1 1>&3)
  lista_de_ficheros | head -n $opcion | tail -n 1
}
