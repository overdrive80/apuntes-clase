...
\begin{homeworkProblem}[: Creación de las tablas]
  ...
  \item Con tu propio usuario, crea las tablas en ese \textit{tablespace}
    \begin{itemize}
    \item Utiliza el \enlace{https://alvarogonzalezsotillo.github.io/sistemas-gestores-bbdd-asir2/apuntes/1/sql/multas.sql}{\textit{script} \texttt{multas.sql}} para la creación de las tablas
...
\begin{homeworkProblem}[: Dar acceso a otros usuarios a un campo de tus tablas]
  ...
  \item Crea sinónimos en todos los usuarios para que puedan acceder a tus tablas sin problemas
    \begin{itemize}
    \item Por ejemplo, el usuario \texttt{profesor} debería poder ejecutar \texttt{SELECT * FROM MULTAS}, puesto que habrás creado un sinónimo del tipo \seqsplit{\texttt{CREATE PUBLIC SYNONYM MULTAS FOR \textit{MIUSUARIO}.MULTAS}}.
    \end{itemize}
  \end{itemize}
...
\begin{homeworkProblem}[: Llena el \textit{tablespace} \texttt{MULTAS}]
  \begin{itemize}
